...
#include "mapniklite_datasource.hpp"
#include "mapniklite_featureset.hpp"
#include "../sqlite/sqlite_resultset.hpp"
#include "../sqlite/sqlite_utils.hpp"
...
void mapniklite_datasource::init() const
{
    ...
    {
        throw datasource_exception("Mapniklite Plugin: " + dataset_name_ + " does not exist");
    }
    ...
    if (table_.empty())
    {
        throw mapnik::datasource_exception("Mapniklite Plugin: missing <table> parameter");
    }
    ...
    {
        MAPNIK_LOG_DEBUG(sqlite) << "mapniklite_datasource: Execute init sql=" << *iter;
    ...
    {
        std::ostringstream s;
        s << "Mapniklite Plugin: could not query table '" << geometry_table_ << "'";
        if (using_subquery_)
        {
    ...
    {
        std::ostringstream s;
        s << "Mapniklite Plugin: unable to detect the column "
    ...
    if (use_spatial_index_)
    {
        ...
        if (!has_spatial_index_ && auto_index)
        {
            ...
            {
                std::ostringstream s;
                s << "Mapniklite Plugin: could not generate spatial index"
    ...
    if (! extent_initialized_)
    {
        ...
        {
            std::ostringstream s;
            s << "Mapniklite Plugin: extent could not be determined for table '"
...
mapniklite_datasource::~mapniklite_datasource()
{
}
...
void mapniklite_datasource::parse_attachdb(std::string const& attachdb) const
{
...
const char * mapniklite_datasource::name()
{
    return "mapniklite";
}

mapnik::datasource::datasource_t mapniklite_datasource::type() const
{
    return type_;
}
    ...
    if (dataset_)
    {
        ...
        while (rs->is_valid() && rs->step_next())
        {
            ...
            if (data)
            {
                ...
                if (mapnik::geometry_utils::from_mapnik_wkb(paths, data, size))
                {
...
featureset_ptr mapniklite_datasource::features(query const& q) const
{
    ...
    if (dataset_)
    {
        ...
        MAPNIK_LOG_DEBUG(sqlite) << "mapniklite_datasource: " << s.str();
